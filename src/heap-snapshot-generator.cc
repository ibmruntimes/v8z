...
void HeapEntry::Print(
  ...
  PrintASCII("%6" V8PRIuPTR " @%6u %*c %s%s: ", self_size(), id(), indent,
                  ' ', prefix, edge_name);
  if (type() != kString) {
    PrintASCII("%s %.40s\n", TypeAsString(), name_);
  } else {
  ...
  for (int i = 0; i < ch.length(); ++i) {
    ...
    switch (edge.type()) {
      ...
      case HeapGraphEdge::kElement:
        SNPrintFASCII(index, "%d", edge.index());
        break;
      ...
      case HeapGraphEdge::kWeak:
        ...
        break;
      default:
        SNPrintFASCII(index, "!!! unknown edge type: %d ", edge.type());
    }
...
bool HeapObjectsMap::MoveObject(Address from, Address to, int object_size) {
  ...
  if (from_value == NULL) {
    ...
    if (FLAG_heap_profiler_trace_objects) {
      PrintASCII("Move object from %p to %p old size %6d new size %6d\n",
             from,
             to,
...
SnapshotObjectId HeapObjectsMap::FindOrAddEntry(Address addr,
  ...
  if (entry->value != NULL) {
    ...
    if (FLAG_heap_profiler_trace_objects) {
      PrintASCII("Update object size : %p with old size %d and new size %d\n",
             addr,
             entry_info.size,
             size);
    }
...
void HeapObjectsMap::UpdateHeapObjectsMap() {
  if (FLAG_heap_profiler_trace_objects) {
    PrintASCII("Begin HeapObjectsMap::UpdateHeapObjectsMap. map has %d entries.\n",
  ...
  for (HeapObject* obj = iterator.next();
    ...
    if (FLAG_heap_profiler_trace_objects) {
      PrintASCII("Update object      : %p %6d. Next address is %p\n",
             obj->address(),
             obj->Size(),
  ...
  RemoveDeadEntries();
  if (FLAG_heap_profiler_trace_objects) {
    PrintASCII("End HeapObjectsMap::UpdateHeapObjectsMap. map has %d entries.\n",
           entries_map_.occupancy());
  }
}
...
struct HeapObjectInfo {
  ...
  void Print() const {
    if (expected_size == 0) {
      PrintASCII("Untracked object   : %p %6d. Next address is %p\n",
             obj->address(),
             obj->Size(),
             obj->address() + obj->Size());
    } else if (obj->Size() != expected_size) {
      PrintASCII("Wrong size %6d: %p %6d. Next address is %p\n",
             expected_size,
             obj->address(),
             obj->Size(),
             obj->address() + obj->Size());
    } else {
      PrintASCII("Good object      : %p %6d. Next address is %p\n",
             obj->address(),
             expected_size,
             obj->address() + obj->Size());
    }
...
int HeapObjectsMap::FindUntrackedObjects() {
  ...
  if (FLAG_heap_profiler_trace_objects) {
    ...
    for (int i = 0; i < heap_objects.length(); ++i) {
      ...
      if (!object_info.IsValid()) {
        ...
        if (last_printed_object != i - 1) {
          if (i > 0) {
            PrintASCII("%d objects were skipped\n", i - 1 - last_printed_object);
    ...
    if (last_printed_object < heap_objects.length() - 1) {
      PrintASCII("Last %d objects were skipped\n",
             heap_objects.length() - 1 - last_printed_object);
    }
    PrintASCII("End HeapObjectsMap::FindUntrackedObjects. %d entries in map.\n\n",
           entries_map_.occupancy());
  }
  return untracked;
}
 ...
 private:
  ...
  void AddNumberImpl(T n, const char* format) {
    ...
    if (chunk_size_ - chunk_pos_ >= kMaxNumberSize) {
      ...
      int result = SNPrintFASCII(buffer, format, n);
      USE(result);
      DCHECK(result != -1);
