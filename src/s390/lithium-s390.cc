...
LInstruction* LChunkBuilder::DoInstanceOf(HInstanceOf* instr) {
  LOperand* context = UseFixed(instr->context(), cp);
  LInstanceOf* result =
      new(zone()) LInstanceOf(context, UseFixed(instr->left(), r2),
...
LInstruction* LChunkBuilder::DoReturn(HReturn* instr) {
  ...
  return new(zone()) LReturn(UseFixed(instr->value(), r2), context,
                             parameter_count);
}
...
LInstruction* LChunkBuilder::DoLoadNamedGeneric(HLoadNamedGeneric* instr) {
  ...
  LInstruction* result =
      DefineFixed(new(zone()) LLoadNamedGeneric(context, object), r2);
  return MarkAsCall(result, instr);
}
...
LInstruction* LChunkBuilder::DoLoadKeyedGeneric(HLoadKeyedGeneric* instr) {
  ...
  LInstruction* result =
      DefineFixed(new(zone()) LLoadKeyedGeneric(context, object, key), r2);
  return MarkAsCall(result, instr);
}
...
LInstruction* LChunkBuilder::DoStringAdd(HStringAdd* instr) {
  ...
  LOperand* right = UseFixed(instr->right(), r2);
  return MarkAsCall(
      DefineFixed(new(zone()) LStringAdd(context, left, right), r2),
      instr);
}
...
LInstruction* LChunkBuilder::DoRegExpLiteral(HRegExpLiteral* instr) {
  LOperand* context = UseFixed(instr->context(), cp);
  return MarkAsCall(
      DefineFixed(new(zone()) LRegExpLiteral(context), r2), instr);
}
...
LInstruction* LChunkBuilder::DoFunctionLiteral(HFunctionLiteral* instr) {
  LOperand* context = UseFixed(instr->context(), cp);
  return MarkAsCall(
      DefineFixed(new(zone()) LFunctionLiteral(context), r2), instr);
}
...
LInstruction* LChunkBuilder::DoTypeof(HTypeof* instr) {
  LOperand* context = UseFixed(instr->context(), cp);
  LTypeof* result = new(zone()) LTypeof(context, UseFixed(instr->value(), r2));
  return MarkAsCall(DefineFixed(result, r2), instr);
}
