...
void FullCodeGenerator::Generate() {
  ...
  if (info->strict_mode() == SLOPPY && !info->is_native()) {
    ...
    __ LoadP(r4, StackMemOperand(sp, receiver_offset), r0);
    ...
    __ StoreP(r4, StackMemOperand(sp, receiver_offset), r0);
    ...
    if (locals_count > 0) {
      ...
      if (locals_count >= kMaxPushes) {
        ...
        for (int i = 0; i < kMaxPushes; i++) {
          __ StoreP(ip, StackMemOperand(sp, i * kPointerSize));
        }
      ...
      if (var->IsContextSlot()) {
        ...
        if (need_write_barrier) {
          __ RecordWriteContextSlot(
            cp, target.offset(), r2, r5, kLRHasBeenSaved, kDontSaveFPRegs);
...
void FullCodeGenerator::StackValueContext::DropAndPlug(int count,
  ...
  __ StoreP(reg, StackMemOperand(sp, 0));
}
...
void FullCodeGenerator::VisitModuleDeclaration(ModuleDeclaration* declaration) {
  ...
  __ LoadContext(r3, scope_->ContextChainLength(scope_->GlobalScope()));
  __ LoadP(r3, ContextOperand(r3, variable->interface()->Index()));
  __ LoadP(r3, ContextOperand(r3, Context::EXTENSION_INDEX));

  // Assign it.
  __ StoreP(r3, ContextOperand(cp, variable->index()), r0);
...
void FullCodeGenerator::VisitSwitchStatement(SwitchStatement* stmt) {
  ...
  for (int i = 0; i < clauses->length(); i++) {
    ...
    __ LoadP(r3, StackMemOperand(sp, 0));  // Switch value.
...
void FullCodeGenerator::VisitForInStatement(ForInStatement* stmt) {
  ...
  __ mov(r4, Operand(TypeFeedbackInfo::MegamorphicSentinel(isolate())));
  ...
  __ LoadP(r4, StackMemOperand(sp, 0 * kPointerSize));  // Get enumerated object
  STATIC_ASSERT(FIRST_JS_PROXY_TYPE == FIRST_SPEC_OBJECT_TYPE);
  ...
  __ LoadP(r2, StackMemOperand(sp, 0 * kPointerSize));
  ...
  __ LoadP(r4, StackMemOperand(sp, 2 * kPointerSize));
  ...
  __ LoadP(r4, StackMemOperand(sp, 3 * kPointerSize));

  // Check if the expected map still matches that of the enumerable.
  // If not, we may have to filter the key.
  Label update_each;
  __ LoadP(r3, StackMemOperand(sp, 4 * kPointerSize));
...
void FullCodeGenerator::EmitNewClosure(Handle<SharedFunctionInfo> info,
  ...
  if (!FLAG_always_opt &&
    ...
    __ LoadRoot(r3, pretenure ? Heap::kTrueValueRootIndex
...
void FullCodeGenerator::EmitVariableLoad(VariableProxy* proxy) {
  ...
  switch (var->location()) {
    case Variable::UNALLOCATED: {
      ...
      // Use inline caching. Variable name is passed in r4 and the global
      // object (receiver) in r2.
      __ LoadP(LoadIC::ReceiverRegister(), GlobalObjectOperand());
...
void FullCodeGenerator::VisitObjectLiteral(ObjectLiteral* expr) {
      ...
      case ObjectLiteral::Property::COMPUTED:
        if (key->value()->IsInternalizedString()) {
          if (property->emit_store()) {
            ...
            __ LoadP(StoreIC::ReceiverRegister(), StackMemOperand(sp));
        ...
        __ LoadP(r2, StackMemOperand(sp));
        __ push(r2);
  ...
  for (AccessorTable::Iterator it = accessor_table.begin();
       ...
       ++it) {
    __ LoadP(r2, StackMemOperand(sp));  // Duplicate receiver.
    __ push(r2);
...
void FullCodeGenerator::VisitArrayLiteral(ArrayLiteral* expr) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (IsFastObjectElementsKind(constant_elements_kind)) {
      ...
      __ LoadP(r7, StackMemOperand(sp, kPointerSize));  // Copy of array literal
      __ LoadP(r3, FieldMemOperand(r7, JSObject::kElementsOffset));
      __ StoreP(result_register(), FieldMemOperand(r3, offset));
      // Update the write barrier for the array store.
      __ RecordWriteField(r3, offset, result_register(), r4,
                          kLRHasBeenSaved, kDontSaveFPRegs,
                          EMIT_REMEMBERED_SET, INLINE_SMI_CHECK);
    } else {
      __ LoadSmiLiteral(r5, Smi::FromInt(i));
  ...
  switch (assign_type) {
    ...
    case NAMED_PROPERTY:
      if (expr->is_compound()) {
        ...
        __ LoadP(LoadIC::ReceiverRegister(), StackMemOperand(sp, 0));
      } else {
        VisitForStackValue(property->obj());
      }
      break;
    case KEYED_PROPERTY:
      if (expr->is_compound()) {
        ...
        __ LoadP(LoadIC::ReceiverRegister(),
                 StackMemOperand(sp, 1 * kPointerSize));
        __ LoadP(LoadIC::NameRegister(), StackMemOperand(sp, 0));
      } else {
        VisitForStackValue(property->obj());
        VisitForStackValue(property->key());
      }
...
void FullCodeGenerator::VisitYield(Yield* expr) {
  ...
  switch (expr->yield_kind()) {
    ...
    case Yield::DELEGATING: {
      ...
      __ LoadP(r5, StackMemOperand(sp, 1 * kPointerSize));   // iter
      ...
      __ LoadP(r2, StackMemOperand(sp, generator_object_depth));
      __ push(r2);                                       // g
      ...
      __ bind(&l_call);
      __ LoadP(load_receiver, StackMemOperand(sp, kPointerSize));
      __ LoadP(load_name, StackMemOperand(sp, 2 * kPointerSize));
      if (FLAG_vector_ics) {
        __ mov(LoadIC::SlotRegister(),
      ...
      __ LoadRR(r3, r2);
      __ StoreP(r3, StackMemOperand(sp, 2 * kPointerSize));
...
void FullCodeGenerator::EmitGeneratorResume(Expression *generator,
  ...
  __ LoadP(r5, FieldMemOperand(r3, JSGeneratorObject::kContinuationOffset));
...
void FullCodeGenerator::EmitCreateIteratorResult(bool done) {
  ...
  __ Allocate(map->instance_size(), r2, r4, r5, &gc_required, TAG_OBJECT);
  __ b(&allocated);
  ...
  // root set.
  __ RecordWriteField(r2, JSGeneratorObject::kResultValuePropertyOffset,
                      r4, r5, kLRHasBeenSaved, kDontSaveFPRegs);
}
...
void FullCodeGenerator::EmitAssignment(Expression* expr) {
  ...
  switch (assign_type) {
    ...
    case NAMED_PROPERTY: {
      ...
      __ Move(StoreIC::ReceiverRegister(), r2);
    ...
    case KEYED_PROPERTY: {
      ...
      __ Move(KeyedStoreIC::NameRegister(), r2);
...
void FullCodeGenerator::EmitStoreToStackLocalOrContextSlot(
  ...
  if (var->IsContextSlot()) {
    ...
    __ RecordWriteContextSlot(
        r3, offset, r5, r4, kLRHasBeenSaved, kDontSaveFPRegs);
  }
}
...
void FullCodeGenerator::EmitVariableAssignment(Variable* var, Token::Value op) {
  if (var->IsUnallocated()) {
    ...
    if (var->IsLookupSlot()) {
      ...
      __ Push(cp, r3, r2);  // Context, name, strict mode.
...
void FullCodeGenerator::EmitKeyedPropertyAssignment(Assignment* expr) {
  ...
  DCHECK(KeyedStoreIC::ValueRegister().is(r2));
...
void FullCodeGenerator::VisitProperty(Property* expr) {
  ...
  if (key->IsPropertyName()) {
    ...
    __ Move(LoadIC::ReceiverRegister(), r2);
...
void FullCodeGenerator::EmitCallWithLoadIC(Call* expr) {
  ...
  if (call_type == CallIC::FUNCTION) {
    ...
    __ LoadP(ip, StackMemOperand(sp, 0));
    __ push(ip);
    __ StoreP(r2, StackMemOperand(sp, kPointerSize));
  }
...
void FullCodeGenerator::EmitCall(Call* expr, CallIC::CallType call_type) {
  ...
  __ LoadP(r3, StackMemOperand(sp, (arg_count + 1) * kPointerSize), r0);
...
void FullCodeGenerator::EmitResolvePossiblyDirectEval(int arg_count) {
  ...
  if (arg_count > 0) {
    __ LoadP(r6, StackMemOperand(sp, arg_count * kPointerSize), r0);
  } else {
...
void FullCodeGenerator::VisitCall(Call* expr) {
  ...
  if (call_type == Call::POSSIBLY_EVAL_CALL) {
    ...
    { PreservePositionScope pos_scope(masm()->positions_recorder());
      ...
      __ StoreP(r2, StackMemOperand(sp, (arg_count + 1) * kPointerSize));
      __ StoreP(r3, StackMemOperand(sp, arg_count * kPointerSize));
    }
    ...
    __ bind(&slow);
    // Call the runtime to find the function to call (returned in r2)
    // and the object holding it (returned in edx).
    DCHECK(!context_register().is(r4));
    __ mov(r4, Operand(proxy->name()));
...
void FullCodeGenerator::VisitCallNew(CallNew* expr) {
  ...
  __ LoadP(r3, StackMemOperand(sp, arg_count * kPointerSize), r0);
  ...
  __ LoadSmiLiteral(r5, Smi::FromInt(expr->CallNewFeedbackSlot()));
...
void FullCodeGenerator::EmitSetValueOf(CallRuntime* expr) {
  ...
  __ LoadRR(r4, r2);
  __ RecordWriteField(
     r3, JSValue::kValueOffset, r4, r5, kLRHasBeenSaved, kDontSaveFPRegs);
...
void FullCodeGenerator::VisitCallRuntime(CallRuntime* expr) {
  ...
  if (expr->is_jsruntime()) {
    ...
    SetSourcePosition(expr->position());
    CallFunctionStub stub(isolate(), arg_count, NO_CALL_FUNCTION_FLAGS);
    __ LoadP(r3, StackMemOperand(sp, (arg_count + 1) * kPointerSize));
    __ CallStub(&stub);
      ...
      if (!context()->IsEffect()) {
        ...
        switch (assign_type) {
          ...
          case NAMED_PROPERTY:
            ...
            break;
          case KEYED_PROPERTY:
            __ StoreP(r2, StackMemOperand(sp, 2 * kPointerSize));
            break;
        }
      }
    }
