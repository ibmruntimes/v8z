...
void Code::Disassemble(const char* name, v8::base::OStream& os) {  // NOLINT
  ...
  if (is_crankshafted()) {
    ...
    for (unsigned i = 0; i < table.length(); i++) {
      ...
      os << v8::base::OStream::setw(4) << std::hex << pc_offset << std::dec << "  ";
    ...
    if (static_cast<int>(offset) < instruction_size()) {
      ...
      for (uint32_t i = 0; i < back_edges.length(); i++) {
        os << v8::base::OStream::setw(6) << back_edges.ast_id(i).ToInt() << "  "
           << v8::base::OStream::setw(9) << std::hex << back_edges.pc_offset(i) << std::dec
           << "  " << v8::base::OStream::setw(10) << back_edges.loop_depth(i) << "\n";
      }
